...
def re_verify(net_vf, img):
    ...
    #print out['prob']
    if out['prob'][0,map_idx] > threshold*0.9:
        return True
    else:
        return False
...
def face_detection_image(net,net_vf,info):
    ...
    true_boxes1 = nms_max(boxes_nms, overlapThresh=0.2)#paras according to experiment
    ...
    rect = mpatches.Rectangle((float(info[2]), float(info[3])),float(info[4]),float(info[5]), fill=False, edgecolor='blue', linewidth=1)
    ax.add_patch(rect)
...
#logging module
    ...
    #count total faces and correct detections
    faces = 0 
    correct = 0
    ...
    while(True):
    #for i in range(2):
        line = imgFile.readline()#next line
        if not line: 
            break
        line = line.strip()
        info = line.split( '\t' )
        ...
        if not os.path.exists( filepath ):#the image file does not exist
            ...
            continue
        result = face_detection_image(net_fc,net_vf,info)
        if(isinstance(result,list)):#09437 returned int
          faces = result[0] + faces
          correct = result[1] + correct
        plt.close('all')

    logging.info('All process done.\n')
    logging.info('All detected faces number: '+str(faces)+'\n' )
    logging.info('Correct number: '+str(correct)+'\n')
